...
int main(int argc, char *argv[])
{
    ...
    if(argc < 2 || argc > 3)
    {
        printf("Usage:\n someprog 2>&1 | %s FIFO [BufferSize]\n"
               ...
               "BufferSize - temporary Internal buffer size of blocks in case write to FIFO fails (resulting to be buffered before discards happen). Example: 16384 blocks uses about 32 Mbyte memory \n", argv[0]);
        exit(EXIT_FAILURE);
    }
